...
function reviewDelete(boardNO){
	location.href="DispatcherServlet?command=reviewdelete&board_no="+boardNO;
}
function reviewUpdate(boardNO){
	location.href="DispatcherServlet?command=reviewupdatepage&board_no="+boardNO;
}
</script>
	...
	<div class="bg-faded p-4 my-4">
		<table class="table table-hover">
			...
			<c:forEach var="bvo" items="${requestScope.lvo.list}">
				...
				<td>${bvo.hit}</td>
				<td><button type="button" class="btn btn-primary" onclick="reviewUpdate(${bvo.board_no})">수정</button></td>
				<td><button type="button" class="btn btn-primary" onclick="reviewDelete(${bvo.board_no})">삭제</button></td>
