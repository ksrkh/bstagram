...
public class LineBoardDAO extends BoardDAO {
	...
	private LineBoardDAO() {
		System.out.println("LineBoardDAO() 객체생성 싱글톤 적용");
	}
	...
	public BoardVO selectBoard(int boardNo) throws SQLException {
		...
		try {
			con = getConnection();
			/*
			 * //sympathy 먼저 처리 후 Board로 set 아래에서 get String
			 * sql1="select count(*) from sympathy where board_no=?";
			 * pstmt=con.prepareStatement(sql1); pstmt.setInt(1, boardNo);
			 * rs=pstmt.executeQuery(); if(rs.next()) { dvo.setSympathy(rs.getInt(1)); }
	 ...
	 * 
	 * SELECT br.board_no, br.boardtype_no, m.id, m.nick, br.board_regdate, br.hit,
	 * br.authority, br.bg_no, lb.line_content, lb.tend_code, lb.book_no FROM
	 * (SELECT row_number() over(order by board_no desc) as
	 * rnum,board_no,line_content,tend_code,book_no FROM line_board) lb, board br,
	 * member m WHERE br.board_no=lb.board_no and br.id=m.id and rnum between 1 and
	 * 5 order by br.board_no desc
	 * 
	 * 페이징 처리 완료
	 */
	@Override
	public ArrayList<BoardVO> boardList(PagingBean pagingBean) throws SQLException {
		...
		try {
			...
			sql.append("FROM line_board) lb, board br, member m ");
			sql.append(
					"WHERE br.board_no=lb.board_no and br.id=m.id and rnum between ? and ?  order by br.board_no desc");
			pstmt = con.prepareStatement(sql.toString());
			pstmt.setInt(1, pagingBean.getStartRowNumber());
			pstmt.setInt(2, pagingBean.getEndRowNumber());
			rs = pstmt.executeQuery();
			/*
			 * int board_no, int boardtype_no, String id, String nick, String board_regdate,
			 * int hit, int sympathy, int authority, int bg_no, String line_content, int
			 * tend_code, int book_no
			 */
			while (rs.next()) {
			...
			// ArrayList<SympathyVO> slist= allLikeCount(pagingBean.getStartRowNumber(),
