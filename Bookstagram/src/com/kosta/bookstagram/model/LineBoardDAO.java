...
public class LineBoardDAO extends BoardDAO {
	...
	public static LineBoardDAO getInstance() {
		return instance;
	}

	/*
	 * 한줄 글등록 LineVO에 ArrayList<LineTendVO> 생성 controller에서 처리 후에 등록
	 */
	@Override
	...
	/*
	 * 한줄 수정하기 update line_board set line_content='asaasd' ,tend_code=3 where
	 * board_no=90;
	 */
	@Override
	...
	/*
	 * 글 번호(검색) 한줄 삭제하기 delete from line_board where board_no=21; delete from reply
	 * where board_no=21; delete from sympathy where board_no=21; delete from board
	 * where board_no=21;
	 * 
	 */
	@Override
	 ...
	 * 
	 * SELECT
	 * br.board_no,br.boardtype_no,m.nick,br.board_regdate,br.hit,br.authority,
	 * br.bg_no,lb.line_content, lb.tend_code,lb.book_no FROM line_board lb,board
	 * br,member m WHERE lb.board_no=br.board_no and m.id=br.id and br.board_no='2';
	 * int board_no, int boardtype_no, String nick, String board_regdate, int hit,
	 * int sympathy, int authority, int bg_no, String line_content, int tend_code,
	 * int book_no **sympathy 수** select count(*) from sympathy where board_no='1'
	 */
	@Override
	public BoardVO selectBoard(int boardNo) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				dvo = new LineBoardVO(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getInt(5),
	...
	/*
	 * 책 게시판 번호로 검색 SELECT bk.book_no,bk.book_title,bk.book_intro,bk.book_author,
	 * bk.book_publ,bk.book_sdate,bk.book_edate,bk.book_cate,bk.book_img FROM
	 * line_board lb,board br,book bk WHERE lb.board_no=br.board_no and
	 * bk.book_no=lb.book_no and br.board_no=?;
	 * 
	 */
	 ...
	 * 
	 * SELECT br.board_no, br.boardtype_no, m.id, m.nick, lb.book_no,
	 * br.board_regdate, br.hit, br.authority, br.bg_no, lb.line_content,
	 * lb.tend_code, lb.book_no, bk.book_title, bk.book_intro, bk.book_author,
	 * bk.book_publ, bk.book_sdate, bk.book_img FROM (SELECT row_number() over(order
	 * by board_no desc) as rnum,board_no,line_content,tend_code,book_no FROM
	 * line_board) lb, board br, member m,book bk WHERE br.board_no=lb.board_no and
	 * br.id=m.id and bk.book_no=lb.book_no and rnum between 1 and 5 order by
	 * br.board_no desc;
	 * 
	 * 페이징 처리 완료
	 */
	public ArrayList<BoardVO> boardList(PagingBean pagingBean) throws SQLException {
		...
		try {
			...
			sql.append(
					"WHERE br.board_no=lb.board_no and br.id=m.id and bk.book_no=lb.book_no and rnum between ? and ? order by br.board_no desc");
			pstmt = con.prepareStatement(sql.toString());
			...
			while (rs.next()) {
				list.add(new LineBoardVO(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5),
						rs.getInt(6), LineBoardDAO.getInstance().likeCount(rs.getShort(1)), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getInt(10), rs.getInt(11), new BookVO(rs.getString(12), rs.getString(13),
	...
	public ArrayList<BoardVO> symCheck(PagingBean pagingBean, String id) throws SQLException {
		ArrayList<BoardVO> list = new ArrayList<BoardVO>();
		...
		try {
			...
			while (rs.next()) {
				list.add(new LineBoardVO(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5),
						rs.getInt(6), LineBoardDAO.getInstance().likeCount(rs.getShort(1)), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getInt(10), rs.getInt(11), sympathyCheck(id, rs.getInt(1)),
						new BookVO(rs.getString(12), rs.getString(13), rs.getString(14), rs.getString(15),
	...
	/*
	 * 성향 찾는 메서드 int tend_code, String tend_name
	 */
	...
	public int sympathyCheck(String id, int board_no) throws SQLException {
		...
		try {
			con = getConnection();
			String sql = "select id from sympathy where board_no=?";
			pstmt = con.prepareStatement(sql);
			...
			for (int i = 0; i < list.size(); i++) {
				if (id.trim().equals(list.get(i).trim()))
					symContains = 1;
			}
		} finally {
			closeAll(rs, pstmt, con);
